...
function Video__Quizindex(quizindex, callback) {
  //	Tony
  console.log(userData.quizData[quizindex - 1]);
  $.ajax({ url: "../vqLib/DAL?video&quizindex=" + quizindex })
    .done(callback)
    .fail(function (e) {
      console.log("quiz_io.js Video__Quizindex() : " + e);
    });
}
...
function reallyLoadQuiz(id) {
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      quizLink =
        userData.serverPath +
        videoPath.substr(3, videoPath.length).split("media")[0];
    ...
    for (var i = 0; i < questions.length; i++) {
      ...
      if (!isQuestionEmpty(q)) {
        ...
        if (q.type == "mc") {
          ...
          if (answerCount < 2) {
            showErrorBubble(
              "Error: Question must have at least 2 answers",
              41.25,
              i,
            );
            return;
          ...
          if (q.answerText[q.correctAnswer - 1].length == 0) {
            showErrorBubble(
              "Error: Correct answer must be non-empty",
              41.25,
              i,
            );
            return;
...
function spawnTrashMarker(i) {
  $("#deleteTrashMarkers").append(
    "<div id='deleteTrashMarker" + i + "' class='deleteTrashMarker'></div>",
  );
  $("#deleteTrashMarker" + i).css("left", 5 + 10 * Math.random() + "%");
  if (i % 2 == 1) {
    $("#deleteTrashMarker" + i).css(
      "background-image",
      "url(img/poke_red.svg)",
    );
  } else {
    $("#deleteTrashMarker" + i).css(
      "background-image",
      "url(img/poke_green.svg)",
    );
  }
}
...
function loadFilter(id) {
  ...
  console.log(
    "quiz_io.js loadFilter() : " + qp.split("/")[1] + " , " + qp.split("/")[2],
  ); //	Tony
  $.ajax({
    //  url: "../vqLib/DAL",					//	Tony
    url: "loadQuiz.php", //	Remove Jim's path-depended loader.	-Tony
...
function refreshFilterList() {
  ...
  for (var i = 0; i < filters.filterList.length; i++) {
    ...
    $("#scoreReportFilterSelect").append(
      '<option value="' + i + '">' + filter.groupName + "</option>",
    );
  }
}
      ...
      if (data !== "" && data !== "{}") {
        ...
        if (canViewData != null && canViewData != undefined) {
          for (var i = 0; i < canViewData.length; i++) {
            viewStr +=
              canViewData[i] + (i < canViewData.length - 1 ? "\n" : "");
          }
        }
