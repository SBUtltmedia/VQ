...
async function loadPermissions() {
  try {
    const response = await fetch('json/permissions.json');
    if (!response.ok) {
    ...
    // Check if the quiz is public or the user has access
    if (state.permissionData.isPublic) {
      state.canView = true;
    } else {
    ...
    // Show/hide blocker UI based on permission
    updateBlockerVisibility();
    
    return state.permissionData;
  } catch (error) {
    console.error('Error loading permissions:', error);
    // Default to allowing view in case of error
    state.canView = true;
...
function checkUserAuthentication() {
  ...
  if (state.permissionData.authorizedUsers) {
    return state.permissionData.authorizedUsers.some(user => 
...
function updateUIFromUserData() {
  ...
  if (userInfoLogin) {
    const userEmail = document.querySelector('meta[name="user-email"]')?.content || 'Guest';
